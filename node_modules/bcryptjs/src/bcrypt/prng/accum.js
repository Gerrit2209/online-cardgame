...
var accum = (function () {
  ...
  function step() {
    if (!to) return;
    if (pool.length >= 255) {
      // stop at 255 values (1 more is added on fetch)
      stop();
      return;
    }
    ...
    if (now % 2) pool[pool.length - 1] += rnd;
    last = now;
  ...
  function start() {
    ...
    if (console.log) console.log("bcrypt-isaac: collecting entropy...");
    // install collectors
  ...
  function fetch() {
    ...
    if (console.log)
      console.log(
        "bcrypt-isaac: using " + res.length + "/256 samples of entropy"
      );
    // console.log(res);
    return res;
  }
  ...
  function mouseCollector(ev) {
    ...
    try {
      ...
      if (x != 0 || y != 0)
        (pool[pool.length - 1] +=
          (x - mouseCollector.last[0]) ^ (y - mouseCollector.last[1])),
          addTime(),
  ...
  function touchCollector(ev) {
    ...
    try {
      ...
      if (x != 0 || y != 0)
        (pool[pool.length - 1] +=
          (x - touchCollector.last[0]) ^ (y - touchCollector.last[1])),
          addTime(),
